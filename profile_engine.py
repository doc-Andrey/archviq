...
def _canonical_no_lag(module, dob: str, name: str) -> Dict[str, Any]:
    ...
    stage = output[1] if isinstance(output, tuple) and len(output) > 1 else None
    cascade = output[2] if isinstance(output, tuple) and len(output) > 2 else None
    if isinstance(raw, pd.Series):
        raw = raw.to_dict()
    if not isinstance(raw, dict):
        raise TypeError("run_one() did not return a result dictionary")

    # Preserve the full non-identifying computational trace for research storage.
    # This does not alter engine 43 mathematics or any production score. The stage
    # table contains window-relative indices/signatures only; the cascade table
    # contains window labels and X-state trajectories. Absolute dates are not added.
    if isinstance(stage, pd.DataFrame):
        raw["_research_stage_signatures"] = stage.to_dict(orient="records")
    if isinstance(cascade, pd.DataFrame):
        raw["_research_cascade_states"] = cascade.to_dict(orient="records")
