...
# Frozen local SILSO archive used by the website. No network download is permitted.
SILSO_ARCHIVE = ROOT / "data" / "SN_d_tot_V2.0.txt"
SILSO_ARCHIVE_SHA256 = "4b57edafb85f28e78bd83a4a1e5273b1a42661691bc60e7826536945a7c490b8"
...
def _verify_silso_archive(path):
    if not path.is_file():
        raise FileNotFoundError(
            f"Frozen SILSO archive is missing: {path}. "
            "Archviq production does not download SSN data from the network."
        )
    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    if digest != SILSO_ARCHIVE_SHA256:
        raise RuntimeError(
            "Frozen SILSO archive SHA256 mismatch. "
            f"Expected {SILSO_ARCHIVE_SHA256}, got {digest}."
        )
    return path


def _load_frozen_silso_raw():
    path = _verify_silso_archive(SILSO_ARCHIVE)
    raw = pd.read_csv(
        path,
        sep=r"\s+",
        header=None,
        names=["year", "month", "day", "decimal_date", "SSN", "std", "n_obs", "definitive"],
        engine="python",
    )
    ...
    return raw[["date", "SSN"]].dropna().copy()
...
def load_ssn():
    # Reproducible, offline-only load from the bundled SILSO archive.
    df = _load_frozen_silso_raw()
