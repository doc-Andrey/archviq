...
from email_report import send_report_email, is_valid_email
from research_store import (
    CONSENT_VERSION, new_id, payload_fingerprint, save_cognitive, save_compatibility,
    save_consent, save_profile, save_questionnaire, storage_configured,
)
APP_BUILD = "FINAL-10 · OLD43 NO-LAG · ANON RESEARCH STORAGE"
...
for k,v in {
    ...
    "privacy_confirmed_mode": None,
    "research_consent": False,
    "research_pair_confirmed": False,
    "research_subject_id": None,
    "research_pair_ids": None,
    "research_pair_id": None,
    "research_logged": {},
    "research_last_error": "",
}.items():
...
if "research_session_id" not in st.session_state:
    st.session_state.research_session_id = new_id()
...
def reset_research_context():
    """Start a fresh anonymous research identity without touching model state."""
    st.session_state.research_session_id = new_id()
    st.session_state.research_subject_id = None
    st.session_state.research_pair_ids = None
    st.session_state.research_pair_id = None
    st.session_state.research_logged = {}
    st.session_state.research_last_error = ""
    st.session_state.privacy_confirmed = False
    st.session_state.privacy_confirmed_mode = None
    st.session_state.research_consent = False
    st.session_state.research_pair_confirmed = False


def _remember_research_result(tag, payload, fn):
    """Write once per session/payload. Storage failures never block the analysis."""
    if not st.session_state.get("research_consent", False):
        return
    fp = payload_fingerprint(payload)
    if st.session_state.research_logged.get(tag) == fp:
        return
    ok, message = fn()
    if ok:
        st.session_state.research_logged[tag] = fp
        st.session_state.research_last_error = ""
    else:
        st.session_state.research_last_error = message


def render_privacy_summary():
    with st.expander(tr("Privacy & research data", "Конфиденциальность и научные данные")):
        st.markdown(tr(
            """**Archviq does not write direct identifiers to its research database.** Your name, email, exact date of birth, sex, IP address, device data and other direct identifiers are not included in the research tables. The exact date of birth is used transiently to calculate the current result and is not persisted in the Archviq research database.

If you opt in, we store only a random anonymous code, numerical engine-43 features and outputs, cognitive-test results, questionnaire scores/answers, and compatibility metrics. These de-identified data may be used for scientific analysis and model improvement. Infrastructure providers such as Streamlit, Vercel or Supabase may maintain their own technical/security logs under their own policies; Archviq does not import those logs into the research dataset.""",
            """**Archviq не записывает прямые идентификаторы в исследовательскую базу.** Имя, email, точная дата рождения, пол, IP-адрес, данные устройства и другие прямые идентификаторы не включаются в исследовательские таблицы. Точная дата рождения используется временно только для расчёта текущего результата и не сохраняется в исследовательской базе Archviq.

При вашем согласии сохраняются только случайный обезличенный код, численные признаки и выходы движка 43, результаты когнитивного теста, опросников и показатели совместимости. Эти обезличенные данные могут использоваться для научного анализа и уточнения модели. Технические провайдеры Streamlit, Vercel или Supabase могут вести собственные технические/защитные журналы по своим правилам; Archviq не переносит эти журналы в исследовательский датасет."""
        ))


def render_privacy_consent():
    """Explicit privacy confirmation + separate research opt-in."""
    # A consent confirmed for personal mode is not silently reused for pair data.
    if st.session_state.get("privacy_confirmed_mode") not in (None, st.session_state.mode):
        st.session_state.privacy_confirmed = False
        st.session_state.research_consent = False
        st.session_state.research_pair_confirmed = False

    st.subheader(tr("Privacy and research use", "Конфиденциальность и использование для исследований"))
    st.info(tr(
        "Your exact birth date is required for the calculation, but Archviq does not persist it in the research database. Research storage is optional.",
        "Точная дата рождения нужна для расчёта, но Archviq не сохраняет её в исследовательской базе. Участие в исследовательском хранении добровольное."
    ))

    if st.session_state.get("privacy_confirmed", False):
        if st.session_state.get("research_consent", False):
            st.success(tr(
                "Confirmed: de-identified results may be stored for research and model improvement.",
                "Подтверждено: обезличенные результаты могут сохраняться для исследований и уточнения модели."
            ))
            if not storage_configured():
                st.warning(tr(
                    "Research storage is not connected yet. Your analysis will still work, but this session cannot be added to the research tables until the backend is configured.",
                    "Исследовательское хранилище пока не подключено. Анализ будет работать, но эта сессия не попадёт в исследовательские таблицы до настройки backend."
                ))
        else:
            st.success(tr(
                "Confirmed: continue without saving results to the Archviq research database.",
                "Подтверждено: продолжить без сохранения результатов в исследовательской базе Archviq."
            ))
        return True

    render_privacy_summary()
    yes = tr(
        "I agree to store my de-identified results for scientific analysis and model improvement",
        "Я согласен(на) на сохранение моих обезличенных результатов для научного анализа и уточнения модели"
    )
    no = tr(
        "Continue without research storage",
        "Продолжить без сохранения данных для исследований"
    )
    choice = st.radio(
        tr("Research-data choice", "Выбор по исследовательским данным"),
        [yes, no],
        index=None,
        key="research_choice_ui",
    )
    pair_ok = True
    if st.session_state.mode == "compat" and choice == yes:
        pair_ok = st.checkbox(tr(
            "I confirm that both participants agree to the use of their de-identified derived results for research.",
            "Подтверждаю, что оба участника согласны на использование их обезличенных производных результатов для исследований."
        ), key="pair_research_ok_ui")
    ack = st.checkbox(tr(
        "I have read the privacy information above and confirm my choice.",
        "Я прочитал(а) информацию о конфиденциальности выше и подтверждаю свой выбор."
    ), key="privacy_ack_ui")
    ready = choice is not None and ack and pair_ok
    if st.button(tr("Confirm and continue", "Подтвердить и продолжить"), type="primary", width="stretch", disabled=not ready, key="privacy_confirm_btn"):
        st.session_state.privacy_confirmed = True
        st.session_state.privacy_confirmed_mode = st.session_state.mode
        st.session_state.research_consent = (choice == yes)
        st.session_state.research_pair_confirmed = bool(pair_ok and st.session_state.mode == "compat" and choice == yes)
        st.session_state.research_subject_id = new_id()
        if st.session_state.mode == "compat":
            st.session_state.research_pair_ids = [new_id(), new_id()]
            st.session_state.research_pair_id = new_id()
        if st.session_state.research_consent:
            _remember_research_result(
                "consent",
                {"consent_version": CONSENT_VERSION, "mode": st.session_state.mode, "pair_ok": st.session_state.research_pair_confirmed},
                lambda: save_consent(
                    session_id=st.session_state.research_session_id,
                    anon_subject_id=st.session_state.research_subject_id,
                    language=L,
                    pair_participant_confirmation=st.session_state.research_pair_confirmed,
                ),
            )
        st.rerun()
    return False
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if not render_privacy_consent():
        if st.button(t("back"), key="privacy_back"):
            st.session_state.step = "landing"
            st.rerun()
        st.stop()

    st.caption(tr(
        "Name is used only to label the current on-screen/PDF result and is not written to the research database.",
        "Имя используется только как подпись текущего результата на экране/PDF и не записывается в исследовательскую базу."
    ))
    ...
    if st.session_state.mode == "personal":
        ...
        if st.button(t("compat_btn"), width="stretch", type="primary"):
            with st.spinner(t("computing")):
                ...
                if st.session_state.get("research_consent", False):
                    if not st.session_state.get("research_pair_ids"):
                        st.session_state.research_pair_ids = [new_id(), new_id()]
                    if not st.session_state.get("research_pair_id"):
                        st.session_state.research_pair_id = new_id()
                    sid1, sid2 = st.session_state.research_pair_ids
                    p1, p2 = st.session_state.p1, st.session_state.p2
                    _remember_research_result(
                        "profile_pair_1", p1.get("raw", {}),
                        lambda: save_profile(
                            session_id=st.session_state.research_session_id, anon_subject_id=sid1, role="pair_1",
                            profile=p1, interpretation=get_interp(p1), app_build=APP_BUILD, language=L,
                        ),
                    )
                    _remember_research_result(
                        "profile_pair_2", p2.get("raw", {}),
                        lambda: save_profile(
                            session_id=st.session_state.research_session_id, anon_subject_id=sid2, role="pair_2",
                            profile=p2, interpretation=get_interp(p2), app_build=APP_BUILD, language=L,
                        ),
                    )
                    compat_now = get_compatibility(p1, p2)
                    raw1, raw2 = p1.get("raw", {}), p2.get("raw", {})
                    deep_now = compatibility_analysis(p1, p2, raw1, raw2, L) if raw1 and raw2 else None
                    _remember_research_result(
                        "compatibility",
                        {"p1": p1.get("raw", {}), "p2": p2.get("raw", {}), "compat": compat_now, "deep": deep_now},
                        lambda: save_compatibility(
                            session_id=st.session_state.research_session_id,
                            pair_id=st.session_state.research_pair_id,
                            subject1_id=sid1, subject2_id=sid2,
                            p1=p1, p2=p2, compatibility=compat_now, deep=deep_now,
                            app_build=APP_BUILD, language=L,
                        ),
                    )
            st.session_state.step = "compat"
            st.rerun()
    if st.session_state.get("research_last_error"):
        st.caption(tr(
            "Research storage status: analysis is working; this research write could not be confirmed.",
            "Статус исследовательского хранилища: анализ работает; запись этой исследовательской строки не подтверждена."
        ))
    if st.button(t("back"), key="input_back"):
        st.session_state.step = "landing"
        st.rerun()
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("#### "+tr("Send the PDF by email","Отправить PDF на email"))
    report_email = st.text_input(
        tr("Email address","Адрес email"),
        placeholder="name@example.com",
        key="report_email",
    )
    st.caption(tr(
        "The email address is used only to deliver this report and is not saved to the ARCHVIQ research database.",
        "Email используется только для отправки этого отчёта и не сохраняется в исследовательской базе ARCHVIQ."
    ))
    if st.button(tr("Send PDF by email","Отправить PDF на email"), key="send_pdf_email", width="stretch"):
        if not is_valid_email(report_email):
            st.warning(tr("Enter a valid email address.","Введите корректный адрес email."))
        else:
            with st.spinner(tr("Sending report...","Отправляем отчёт...")):
                try:
                    send_report_email(report_email, pdf_bytes, lang=L)
                    st.success(tr("Report sent.","Отчёт отправлен."))
                except Exception:
                    st.error(tr(
                        "The report could not be sent. Please try again in a moment.",
                        "Не удалось отправить отчёт. Попробуйте ещё раз через несколько секунд."
                    ))
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if uploaded is not None:
        try:
            ...
            if st.session_state.get("research_consent", False):
                gap_rows = cognitive_gap_rows(st.session_state.p1, cognitive_result)
                _remember_research_result(
                    "cognitive",
                    cognitive_result,
                    lambda: save_cognitive(
                        session_id=st.session_state.research_session_id,
                        anon_subject_id=st.session_state.research_subject_id,
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if c1.button(t("submit_quiz"), type="primary", width="stretch"):
        ...
        if st.session_state.get("research_consent", False):
            score_pct_now, _ = score_quiz(answers, quiz_type)
            domains_now = quiz_domain_scores(answers, quiz_type)
            _remember_research_result(
                f"questionnaire_{quiz_type}",
                {"answers": answers, "score": score_pct_now, "domains": domains_now},
                lambda: save_questionnaire(
                    session_id=st.session_state.research_session_id,
                    anon_subject_id=st.session_state.research_subject_id,
                    questionnaire_type=quiz_type,
                    answers=answers,
                    score_pct=score_pct_now,
                    domains=domains_now,
                    app_build=APP_BUILD,
                    language=L,
                ),
