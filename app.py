...
APP_BUILD = "FINAL-11 · OLD43 NO-LAG · SIMPLE RESEARCH V2 · FROZEN SILSO"
...
def render_privacy_summary():
    with st.expander(tr("Privacy & research data", "Конфиденциальность и научные данные")):
        st.markdown(tr(
            """**Archviq does not write direct identifiers to its research database.** Your name, email, exact date of birth, sex, IP address and device data are not written to the Archviq research table. The exact date of birth is used transiently for the current calculation and is then discarded.

If you opt in, we store a random research code, an estimated conception date used for developmental-window research, numerical engine-43 features and outputs, cognitive-test results, questionnaire scores/answers, and compatibility metrics. Because an exact derived conception date is retained, the research dataset is pseudonymized rather than guaranteed anonymous. It is used only for scientific analysis and model improvement. Infrastructure providers such as Streamlit, Vercel or Supabase may maintain their own technical/security logs under their own policies; Archviq does not import those logs into the research dataset.""",
            """**Archviq не записывает прямые идентификаторы в исследовательскую базу.** Имя, email, точная дата рождения, пол, IP-адрес и данные устройства не записываются в исследовательскую таблицу Archviq. Точная дата рождения используется временно для текущего расчёта и затем отбрасывается.

При вашем согласии сохраняются случайный исследовательский код, расчётная дата зачатия для анализа развивающих окон, численные признаки и выходы движка 43, результаты когнитивного теста, ответы/шкалы опросников и показатели совместимости. Поскольку сохраняется точная производная дата зачатия, корректно считать этот набор псевдонимизированным, а не гарантированно анонимным. Данные используются только для научного анализа и уточнения модели. Технические провайдеры Streamlit, Vercel или Supabase могут вести собственные технические/защитные журналы по своим правилам; Archviq не переносит эти журналы в исследовательский датасет."""
        ))
...
def render_privacy_consent():
    ...
    st.info(tr(
        "Your exact birth date is required for the calculation but is not stored. If you consent to research use, an estimated conception date and the derived results are stored under a random research code. Research storage is optional.",
        "Точная дата рождения нужна для расчёта, но не сохраняется. При согласии на исследовательское использование под случайным кодом сохраняются расчётная дата зачатия и производные результаты. Участие добровольное."
    ))
    ...
    yes = tr(
        "I agree to store my pseudonymized research results, including the estimated conception date, for scientific analysis and model improvement",
        "Я согласен(на) на сохранение моих псевдонимизированных исследовательских результатов, включая расчётную дату зачатия, для научного анализа и уточнения модели"
    )
    no = tr(
