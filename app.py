...
.price-card{min-height:205px;background:linear-gradient(145deg,rgba(22,31,55,.98),rgba(12,18,34,.96));border:1px solid rgba(111,216,196,.28);border-radius:18px;padding:1.25rem;margin:.4rem 0;box-shadow:0 12px 32px rgba(0,0,0,.20)}.price-card b{font:600 1.08rem 'Newsreader',serif;color:#FFFFFF}.price{font:700 2.15rem 'IBM Plex Mono',monospace;color:#F3BB63;margin:.75rem 0}.price-card p{color:#D8DEEA;font-size:.90rem;line-height:1.55}.price-note{font-size:.78rem;color:#BFC8D8;margin-top:.55rem}
...
def render_pricing():
    ...
    for col,(name,desc,price,key) in zip(cols,offers):
        with col:
            st.markdown(
                f'<div class="price-card"><b>{name}</b><div class="price">{price}</div>'
                f'<p>{desc}</p></div>',
                unsafe_allow_html=True
            )
            if key:
                purchase_button(tr(f"Buy · {price}",f"Купить · {price}"),key)
            else:
                st.markdown(
                    '<div style="text-align:center;padding:.72rem;border:1.5px solid rgba(227,163,78,.72);'
                    'border-radius:12px;color:#FFE0A5;font-weight:750;font-size:.95rem;'
                    'background:rgba(227,163,78,.12)">✓ '+tr("Included","Включено")+'</div>',
                    unsafe_allow_html=True,
                )
    ...
    if paywall_gate("compatibility","compat_unlocked",PRICES["compatibility"]):
        ...
        for col,person in ((c1,p1),(c2,p2)):
            with col:
                ...
                st.markdown(
                    f'<div class="pair-card"><div class="eyebrow">{person.get("type_name","")}</div>'
