...
APP_VERSION = "ARCHVIQ WEB 2.0 · ENGINE 43 v1.0"
...
st.set_page_config(
    page_title="ARCHVIQ — Your brain's operating map",
    page_icon="◉",
    layout="wide",
...
:root{
  --bg:#071019;--bg2:#0A1622;--panel:#0E1C29;--panel2:#102433;
  --ink:#ECF3F7;--muted:#90A7B5;--cyan:#64D7C5;--gold:#E7B45A;
  --line:rgba(160,190,205,.16);--good:#8FE3A2;--danger:#F2A88E;
}
...
h1,h2,h3,.brand{font-family:'Space Grotesk',sans-serif!important;letter-spacing:-.03em}
h1{font-size:clamp(2.6rem,7vw,5.5rem)!important;line-height:.98!important;margin-bottom:1rem!important}
h2{font-size:clamp(1.8rem,4vw,3rem)!important;margin-top:1.4rem!important}
p,li{line-height:1.65}
.kicker{font-family:'IBM Plex Mono',monospace;color:var(--cyan);font-size:.76rem;letter-spacing:.13em;text-transform:uppercase;margin-bottom:.6rem}
.hero-copy{font-size:clamp(1.03rem,2vw,1.35rem);max-width:760px;color:#C3D2DA;line-height:1.6}
.hero-accent{color:var(--gold)}
.card{background:linear-gradient(145deg,rgba(16,36,51,.93),rgba(11,25,37,.96));border:1px solid var(--line);border-radius:18px;padding:1.25rem 1.35rem;height:100%;box-shadow:0 20px 70px rgba(0,0,0,.16)}
.card h3{font-size:1.05rem;margin:.1rem 0 .55rem}.card p{color:var(--muted);font-size:.93rem;margin:.1rem 0}
.metric-card{border-top:2px solid var(--cyan);background:rgba(14,28,41,.86);border-radius:12px;padding:1rem 1.1rem;margin:.35rem 0}
.metric-number{font:600 1.8rem 'IBM Plex Mono',monospace;color:#fff}.metric-name{color:var(--cyan);font-weight:600}.metric-note{font-size:.8rem;color:var(--muted)}
.rule{height:1px;background:linear-gradient(90deg,transparent,var(--line),transparent);margin:3rem 0}
.formula{font:500 .9rem/1.75 'IBM Plex Mono',monospace;color:#CDEFE7;background:#07111B;border-left:3px solid var(--cyan);padding:1rem 1.2rem;border-radius:4px 14px 14px 4px;margin:1rem 0}
.pipe{display:grid;grid-template-columns:repeat(5,1fr);gap:.7rem;margin:1.2rem 0}.pipe div{border:1px solid var(--line);border-radius:14px;padding:1rem;text-align:center;background:rgba(14,28,41,.7);color:var(--muted);font-size:.84rem}.pipe b{display:block;color:var(--gold);font:500 1.1rem 'IBM Plex Mono',monospace;margin-bottom:.35rem}
...
.badge{display:inline-block;padding:.28rem .55rem;border:1px solid var(--line);border-radius:999px;color:var(--muted);font:500 .72rem 'IBM Plex Mono',monospace;margin:.15rem .25rem .15rem 0}
.price-card{background:linear-gradient(145deg,rgba(16,36,51,.95),rgba(11,25,37,.96));border:1px solid var(--line);border-radius:18px;padding:1.15rem;min-height:190px}.price{font:600 2rem 'IBM Plex Mono',monospace;color:var(--gold);margin:.6rem 0}.price-card p{font-size:.86rem;color:var(--muted)}
.small{font-size:.78rem;color:var(--muted)}
[data-testid="stForm"]{background:rgba(14,28,41,.58);border:1px solid var(--line);border-radius:18px;padding:1.1rem}
[data-testid="stMetric"]{background:rgba(14,28,41,.72);border:1px solid var(--line);padding:1rem;border-radius:14px}
[data-testid="stExpander"]{background:rgba(14,28,41,.55);border:1px solid var(--line);border-radius:14px}
...
def topbar():
    c1, c2, c3 = st.columns([1.2, 2.5, .8])
    with c1:
        st.markdown('<div class="brand" style="font-size:1.4rem;font-weight:700">ARCHVIQ<span style="color:#64D7C5">.</span></div>', unsafe_allow_html=True)
    with c2:
        nav = st.radio(
            "nav",
            ...
            index=["home", "profile", "method", "stage2"].index(st.session_state.page if st.session_state.page in {"home","profile","method","stage2"} else "home"),
            key="nav_radio",
        )
        if nav != st.session_state.page:
            ...
            st.rerun()
    with c3:
        lang = st.radio("lang", ["RU", "EN"], horizontal=True, label_visibility="collapsed", index=0 if st.session_state.lang == "RU" else 1)
...
def render_prices():
    st.markdown('<div class="kicker">ARCHVIQ products</div>', unsafe_allow_html=True)
    st.subheader(tr("Choose how deep you want to go", "Выберите глубину анализа"))
    offers = [
        (tr("Architecture snapshot", "Архитектурный профиль"), "FREE", tr("Date-linked profile, seven operating axes and timing stability.", "Профиль по дате, семь рабочих осей и устойчивость к неопределённости времени зачатия."), None),
        ...
        (tr("Target questionnaire", "Целевой опросник"), "$19", tr("Compatibility, burnout or AI-work profile.", "Совместимость, выгорание или профиль работы с ИИ."), PRODUCTS["burnout"]),
        (tr("Full Architecture Report", "Полный Architecture Report"), "$39", tr("Integrated profile, cognition, questionnaires and personalized operating strategy.", "Интегрированный профиль, когнитивные показатели, опросники и персональная стратегия работы."), PRODUCTS["full"]),
    ]
    cols = st.columns(4)
    for col, (name, price, desc, url) in zip(cols, offers):
        with col:
            st.markdown(f'<div class="price-card"><h3>{name}</h3><div class="price">{price}</div><p>{desc}</p></div>', unsafe_allow_html=True)
            if url:
                st.link_button(tr("Open", "Открыть"), url, use_container_width=True)
            else:
...
def render_home():
    ...
    with left:
        ...
        with b2:
            if st.button(tr("See the calculation logic", "Посмотреть логику расчёта"), use_container_width=True):
                goto("method")
        st.markdown('<div style="margin-top:1.1rem"><span class="badge">NO-LAG</span><span class="badge">SILSO DAILY DATA</span><span class="badge">15 WINDOWS</span><span class="badge">X9 CASCADE</span><span class="badge">TIMING UNCERTAINTY</span></div>', unsafe_allow_html=True)
    with right:
        st.markdown(f"""
        <div class="card" style="margin-top:1rem">
          ...
          <div class="formula">RESOURCE · SWITCHING · LOCK · NOVELTY · CONTROL · PROCESSING COST · MATURATION</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div class="rule"></div>', unsafe_allow_html=True)
    st.markdown('<div class="kicker">ONE SYSTEM · THREE LEVELS</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="rule"></div>', unsafe_allow_html=True)
    render_pipeline()
    st.markdown(f'<div class="formula">{tr("DOB → conception ensemble → W0…W5 + N0…P8 → SSN dynamics → phase transitions → X9 cascade → operating axes → cognitive / questionnaire / EEG calibration", "DOB → ансамбль дат зачатия → W0…W5 + N0…P8 → динамика SSN → переходы фаз → каскад X9 → рабочие оси → калибровка тестами / опросниками / EEG")}</div>', unsafe_allow_html=True)
...
def render_founder():
    ...
    st.markdown('<div class="kicker">FOUNDER</div>', unsafe_allow_html=True)
    st.markdown('<div class="founder">', unsafe_allow_html=True)
    c1, c2 = st.columns([.7,1.7], gap="large")
    with c1:
        if photo.exists():
            st.image(str(photo), use_container_width=True)
    with c2:
        st.subheader("Andrey Osipov, MD, PhD")
        st.markdown(tr(
            "Physician-scientist and creator of ARCHVIQ. More than 15 years in clinical medicine, PhD work in hemostasis and pregnancy, healthcare management and pharmaceutical industry experience. The project combines developmental neurobiology, neurophysiology, EEG, cognitive measurement and computational modeling.",
            "Врач и автор ARCHVIQ. Более 15 лет клинической медицины, кандидатская диссертация по гемостазу при беременности, опыт управления в здравоохранении и фармацевтической отрасли. Проект объединяет нейробиологию развития, нейрофизиологию, EEG, когнитивные измерения и вычислительное моделирование."
        ))
...
def score_chart(profile):
    ...
    fig = go.Figure(go.Scatterpolar(
        ...
        line=dict(color="#64D7C5", width=3), fillcolor="rgba(100,215,197,.20)"
    ))
    fig.update_layout(
        template="plotly_dark", height=460, margin=dict(l=40,r=40,t=40,b=30),
        paper_bgcolor="rgba(0,0,0,0)",
        polar=dict(bgcolor="rgba(14,28,41,.42)", radialaxis=dict(range=[0,100],gridcolor="rgba(160,190,205,.16)")),
        showlegend=False,
    )
...
def x9_chart(profile):
    ...
    vals = list(x.values())
    fig=go.Figure(go.Bar(x=names,y=vals,marker_color="#E7B45A"))
    fig.update_layout(template="plotly_dark",height=330,margin=dict(l=15,r=15,t=30,b=25),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(14,28,41,.42)",yaxis=dict(range=[0,100],gridcolor="rgba(160,190,205,.12)"),xaxis_title="X9")
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})


def render_result(profile):
    interp = interpret(profile, st.session_state.lang)
    st.markdown('<div class="kicker">YOUR ARCHVIQ PROFILE</div>', unsafe_allow_html=True)
    st.title(f"{profile['name']} · {interp['title']}")
    st.markdown(f'<div class="hero-copy">{interp["subtitle"]}</div>', unsafe_allow_html=True)
    st.caption(f"{profile['engine_version']} · DOB {profile['dob']} · central conception anchor {profile['central_conception']} · NO-LAG")
    ...
    with right:
        ...
        st.metric(tr("Largest timing range","Максимальная чувствительность ко времени"),LABELS[st.session_state.lang][highest_unc[0]],f"±{highest_unc[1]/2:.1f} pts")
        st.markdown(tr(
    ...
    for pair in rows:
        ...
        for col,item in zip(cols,pair):
            with col:
                ...
                st.markdown(f'<div class="metric-card"><div class="metric-name">{item["label"]}</div><div class="metric-number">{item["value"]:.1f}</div><div class="metric-note">{tr("timing range","диапазон по времени")}: {rng:.1f} pts</div><p>{item["text"]}</p></div>',unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.markdown(tr(
            ...
            "Архитектурный слой становится наиболее полезным в паре с реальной производительностью. Stage 2 измеряет простую реакцию, выбор, рабочую память, Simon-интерференцию и скорость/точность сложного правила. Разница между архитектурой и фактической работой становится слоем GAP."
        ))
        if st.button(tr("Open cognitive calibration", "Открыть когнитивную калибровку"), type="primary", use_container_width=True):
            goto("stage2")
    with c2:
        st.link_button(tr("Full Architecture Report · $39", "Полный Architecture Report · $39"), PRODUCTS["full"], use_container_width=True)
    ...
    with st.expander(tr("Advanced: X9 cascade", "Продвинутый уровень: каскад X9")):
        ...
        st.markdown(tr(
            ...
            "X9 — компактный нелинейный слой между динамикой окон и практической картой из семи осей. Он сохраняется как промежуточное представление для последующего сопоставления с когнитивными и EEG-показателями."
        ))
...
def render_profile():
    ...
    with st.form("profile_form"):
        ...
        submit=st.form_submit_button(tr("Calculate with Engine 43","Рассчитать Engine 43"),type="primary",use_container_width=True)
    if submit:
        if not accepted:
...
def render_method():
    st.markdown('<div class="kicker">ENGINE 43 · CALCULATION LOGIC</div>',unsafe_allow_html=True)
    ...
    st.markdown(tr(
        ...
        "Центральная оценка зачатия — DOB − 266 дней. Поскольку реальная длительность беременности различается, ARCHVIQ повторяет тот же расчёт также для 252, 259, 273 и 280 дней. Центральное значение остаётся 266; соседние точки измеряют чувствительность к неопределённости времени."
    ))
    st.markdown('<div class="formula">CTB = {252, 259, 266, 273, 280} days · central = 266 · no client-specific optimization</div>',unsafe_allow_html=True)
    ...
    defs=[
        ...
        ("JERK",tr("third-order change / jerk","изменение ускорения / jerk")),
    ...
    st.markdown(tr(
        ...
        "Окна сводятся в раннюю пренатальную, позднюю пренатальную и постнатальную фазы. Движок также сохраняет межфазные дельты и ключевые локальные переходы (W1→W2, W2→W3, W4→W5, W5→N0). Они поступают в девять нелинейных координат: возбуждение, чувствительность, стабильность, интеграция, гибкость, лабильность, сегрегация, hub-нагрузка и созревание."
    ))
    st.markdown('<div class="formula">window physics → phase summaries + transitions → X_EXC · X_SENS · X_STAB · X_INTEG · X_FLEX · X_LAB · X_SEGR · X_HUB · X_MAT</div>',unsafe_allow_html=True)
    ...
    st.markdown(tr(
        ...
        "Каскад X9 сжимается в семь клиентских осей: Ресурс, Переключение, Фиксация, Новизна, Контроль, Цена обработки и Созревание. Этот слой предназначен для прямого сопоставления со Stage 2, опросниками и при необходимости EEG."
    ))
...
def render_stage2():
    ...
    if html_path.exists():
        ...
        st.error("cognitive_test.html missing")
    st.markdown('<div class="rule"></div>',unsafe_allow_html=True)
    st.link_button(tr("Get Cognitive + GAP report · $12","Получить Cognitive + GAP · $12"),PRODUCTS["gap"],use_container_width=True)
