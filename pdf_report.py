...
# Unicode fonts: built-in Helvetica has no Cyrillic glyphs.
def _register_unicode_fonts():
    variants = {
        "ARCHVIQSans": font_manager.FontProperties(family="DejaVu Sans", weight="normal", style="normal"),
        "ARCHVIQSans-Bold": font_manager.FontProperties(family="DejaVu Sans", weight="bold", style="normal"),
        "ARCHVIQSans-Oblique": font_manager.FontProperties(family="DejaVu Sans", weight="normal", style="italic"),
        "ARCHVIQSans-BoldOblique": font_manager.FontProperties(family="DejaVu Sans", weight="bold", style="italic"),
    }
    for name, props in variants.items():
        if name not in pdfmetrics.getRegisteredFontNames():
            pdfmetrics.registerFont(TTFont(name, font_manager.findfont(props, fallback_to_default=True)))
    pdfmetrics.registerFontFamily(
        "ARCHVIQSans", normal="ARCHVIQSans", bold="ARCHVIQSans-Bold",
        italic="ARCHVIQSans-Oblique", boldItalic="ARCHVIQSans-BoldOblique"
    )

_register_unicode_fonts()

STYLES = {
    "title": ParagraphStyle("title", fontName="ARCHVIQSans-Bold", fontSize=22,
                             leading=26, textColor=NAVY, spaceAfter=4),
    "tagline": ParagraphStyle("tagline", fontName="ARCHVIQSans-Oblique", fontSize=12,
                               leading=16, textColor=MUTED, spaceAfter=18),
    "h2": ParagraphStyle("h2", fontName="ARCHVIQSans-Bold", fontSize=14,
                          leading=18, textColor=NAVY, spaceBefore=18, spaceAfter=8),
    "body": ParagraphStyle("body", fontName="ARCHVIQSans", fontSize=10.5,
                            leading=15, textColor=colors.HexColor("#1A1F2B"), spaceAfter=6),
    "muted": ParagraphStyle("muted", fontName="ARCHVIQSans", fontSize=9,
                             leading=13, textColor=MUTED),
    "meta": ParagraphStyle("meta", fontName="ARCHVIQSans", fontSize=9,
                            leading=13, textColor=MUTED, alignment=TA_LEFT),
    "index_title": ParagraphStyle("index_title", fontName="ARCHVIQSans-Bold", fontSize=11,
                                   leading=14, textColor=NAVY),
    "index_desc": ParagraphStyle("index_desc", fontName="ARCHVIQSans", fontSize=9.5,
                                  leading=13.5, textColor=colors.HexColor("#1A1F2B")),
    "footer": ParagraphStyle("footer", fontName="ARCHVIQSans", fontSize=8,
...
def _axis_row(label, value):
    ...
    t.setStyle(TableStyle([
        ("FONTNAME", (0, 0), (0, 0), "ARCHVIQSans-Bold"),
...
def build_pdf(profile: dict, interp: dict | None = None, lang: str = "RU") -> bytes:
    ...
    story.append(Paragraph("ARCHVIQ", ParagraphStyle(
        "brand", fontName="ARCHVIQSans-Bold", fontSize=13, textColor=GOLD, spaceAfter=2)))
